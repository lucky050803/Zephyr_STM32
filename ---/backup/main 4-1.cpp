...
//K_MSGQ_DEFINE(my_msgq, sizeof(struct message), 10, 1);
char my_msgq_buffer[MESSAGE_QUEUE_SIZE * sizeof(struct data_item_type )];
...
static void display_task(void *p1, void *p2, void *p3)
{
    ...
    while (1)
    {
        ...
        sprintf(msg.text, "%d.%02d - %d.%02d", bme680.temperature.val1, bme680.temperature.val2 / 10000, bme680.humidity.val1, bme680.humidity.val2 / 10000);
        //display.text_add(msg.text);
        while (k_msgq_put(&my_msgq, &msg, K_NO_WAIT) != 0) {
...
int main(void)
{
    ...
    k_tid_t bme680_tid;
    struct k_thread k_msg;
    k_tid_t k_msg_tid;
    struct k_thread k_hello;
    k_tid_t k_hello_tid;
    ...
    k_msg_tid = k_thread_create(&k_msg, msg_stack, K_THREAD_STACK_SIZEOF(msg_stack),
                    message_display_task, NULL, NULL, NULL, 
                    PRIO_MSG_TASK, 0, K_NO_WAIT);
    k_thread_name_set(k_msg_tid, "kmsg");

    k_hello_tid = k_thread_create(&k_hello, hello_stack, K_THREAD_STACK_SIZEOF(msg_stack),
                    hello_task, NULL, NULL, NULL, 
                    PRIO_hello_TASK, 0, K_NO_WAIT);
    k_thread_name_set(k_hello_tid, "hello");
}