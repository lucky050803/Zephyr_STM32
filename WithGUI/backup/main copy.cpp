...
static void write_fnct(char* text_to_add)
{
    if (strlen(display.text_value) > TEXT_MAX_SIZE - strlen(text_to_add) - 2 || display.text_nb > display.text_nb_max)
    {
        display.text_clear();
    }
    display.text_nb++;
    strcat(display.text_value, text_to_add);
    strcat(display.text_value, "\n");
    lv_label_set_text(display.text, display.text_value);
}
